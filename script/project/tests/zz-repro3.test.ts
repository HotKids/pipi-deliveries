...
function carrierPackage(provider: string, complete: boolean) {
  return {
    provider, complete, structuredStatus: true, waybill: WAYBILL,
...
const feed = {
  provider: "interface5", complete: false, structuredStatus: true, waybill: WAYBILL,
  courierCode: "ZTO", companyName: "中通快递", semantic: "COMPLETED",
...
const stored = {
  ...
  route: null, accountRecord: null, updatedAtMs: T - 600_000,
} as any;
...
console.log("initial manuals:", (initial.shipments[0].manualTimelines || []).map((t: any) => t.provider));

const parcel = {
  source: "interface5", ownerId: WAYBILL, waybill: WAYBILL, orderId: "", accountOrder: false,
  ...
  latestTimeText: "", latestDetail: "", tracks: [], routeUrl: "", projectionUrl: "",
} as any;

const merged = mergeAccountParcel(initial, initial.shipments, parcel, ["13800001515"], "interface5", T + 2, new Map());
console.log("merged manuals:", (merged[0].manualTimelines || []).map((t: any) => t.provider));
console.log("merged timeline:", merged[0].timeline.provider, merged[0].timeline.tracks.length);

const commit = commitRefreshState(initial, { ...initial, shipments: merged } as any, "interface5", T + 3);
console.log("applied", commit.applied);
const after = commit.state.shipments[0];
console.log("committed manuals:", (after.manualTimelines || []).map((t: any) => t.provider));
console.log("committed timeline:", after.timeline.provider, after.timeline.tracks.length);
console.log("detail:", selectShipmentDetailTimeline(after).provider, selectShipmentDetailTimeline(after).tracks.length);
