                ...
                if ("interface5".equals(queryBindingSource)) {
                    result = ManualQueryCoordinator.query(
                            ...
                            () -> new ExpressApi(getApplicationContext())
                                    .queryWithPhones(waybill, courierHint, phones));
                } else {
                    result = ManualQueryCoordinator.query(
    ...
    private void scheduleCarrierDetection() {
        ...
        carrierDetectStart = () -> {
            ...
            carrierDetectTask = carrierDetectWorker.submit(() ->
                    detectCarrier(waybill, generation));
        };
        mainHandler.postDelayed(carrierDetectStart, CARRIER_DETECT_DELAY_MS);
    }

    private void detectCarrier(String waybill, long generation) {
        String code = "";
        String company = "";
        try {
            ...
            if (cached != null) {
                ...
                if (ExpressAccountSource.isV5(this)) {
                    ...
                    code = new ExpressApi(getApplicationContext()).detect(waybill);
        ...
        if (generation != carrierDetectGeneration || !waybill.equals(detectedWaybill)) return;
        detectedCourierCode = code;
        String shownCompany = company;
        runOnUiThread(() -> {
            if (isFinishing() || isDestroyed()
                    || generation != carrierDetectGeneration
                    || !waybill.equals(currentWaybill())) return;
            updateCarrierSuffix(shownCompany, false, shownCompany.isEmpty());
        });
    }
