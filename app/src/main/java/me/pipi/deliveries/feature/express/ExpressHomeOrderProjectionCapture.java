...
final class ExpressHomeOrderProjectionCapture {
    ...
    private void accept(WebView target, String projectionJson) {
        if (!current(target)) return;
        ...
        try {
            ...
            if (!waybill.matches("^[A-Za-z0-9_-]{6,40}$")
                    || ExpressDetailActivity.normalizeIdentity(waybill).equals(
                    ExpressDetailActivity.normalizeIdentity(source.waybill))) return;
            ...
            boolean saved = repository.saveOrderProjection(
                    source.waybill, bindingSource, waybill, company, "[]");
    ...
    private boolean sameUnresolvedSource(ExpressItem current) {
        if (!needsProjection(current)) return false;
        String owner = current.stateOwner.isEmpty() ? current.source : current.stateOwner;
        return current.rowId == source.rowId
                && bindingSource.equals(ExpressAccountSource.bindingSourceForOwner(owner))
                && ExpressDetailActivity.normalizeIdentity(current.waybill).equals(
                ExpressDetailActivity.normalizeIdentity(source.waybill))
                && ExpressDetailActivity.safeOrderH5Url(current).equals(
                ExpressDetailActivity.safeOrderH5Url(source));
    }
