...
final class ExpressWidgetPresentation {
    ...
    /** Requested delivery priority first, followed by the remaining visible states. */
    static PriorityStatus priorityStatus(List<ExpressItem> items) {
        for (StatusSemantic semantic : new StatusSemantic[]{
            ...
            int count = 0;
            for (ExpressItem item : items) {
                boolean preTransit = semantic == StatusSemantic.PICKED
                        && (item.semantic == StatusSemantic.SHIPPED
                        || item.semantic == StatusSemantic.ORDERED);
                if (item.semantic == semantic || preTransit) {
                    count++;
                }
            }
            if (count > 0) return new PriorityStatus(semantic, count);
        }
        return new PriorityStatus(StatusSemantic.WAITING_PICKUP, 0);
    }
