...
public final class ExpressSubscriptionClient {
    ...
    public void bind(Context context, String phone, String code) throws Exception {
        JSONObject result = requestBind(context, phone, code);
    ...
    public List<ExpressQueryResult> query(Context context) throws Exception {
        ...
        if (!response.successful()) throw new IllegalStateException("快递同步失败");
        Object root = unwrap(response.utf8());
    ...
    public ExpressQueryResult queryWaybill(
        ...
        if (!response.successful()) throw new IllegalStateException("主接口刷新失败");
        Object root = unwrap(response.utf8());
    ...
    private JSONObject requestBind(Context context, String phone, String code) throws Exception {
        ...
        JSONObject result = new JSONObject(response.utf8());
        int responseCode = result.optInt("code", result.optInt("status", -1));
        Log.i(TAG, operation + " http=" + response.status + " code=" + responseCode
                + " message=" + safeLogMessage(message(result)));
        if (responseCode != 0 && responseCode != 200) {
            throw new IllegalStateException(message(result));
        }
        return result;
    ...
    private static Object unwrap(String body) throws Exception {
        ...
        if (envelope.has("code")) {
            ...
            if (code != 0 && code != 200) throw new IllegalStateException(message(envelope));
        }
    ...
    private static String message(JSONObject value) {
        String message = value.optString("message", value.optString("msg", ""));
