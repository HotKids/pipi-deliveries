...
public final class ExpressDiscoveryClient {
    ...
    private static void requestAccount(
        ...
        if (!response.successful()) throw new IllegalStateException("网络异常，请稍后重试");
        JSONObject outer = new JSONObject(response.utf8());
        if (outer.optInt("code", -1) != 0) {
            throw new IllegalStateException(
                    outer.optString("message", "验证失败，请稍后重试"));
        }
    }
    ...
    public int sync(Context context, List<String> phones) throws Exception {
        ...
        if (!response.successful()) {
            String message = "";
            try {
                message = new JSONObject(response.utf8()).optString("message", "").trim();
            } catch (Throwable ignored) {
                // The HTTP status is sufficient when the gateway did not return JSON.
            }
            Log.w(TAG, "Discovery HTTP " + response.status
                    + (message.isEmpty() ? "" : ": " + message));
            throw new IllegalStateException("同步失败，请稍后重试");
        }
        JSONObject outer = new JSONObject(response.utf8());
        if (outer.optInt("code", -1) != 0) {
            String message = outer.optString("message", "同步失败，请稍后重试");
    ...
    public CarrierMatch detectManualCarrier(Context context, String waybill) throws Exception {
        ...
        if (!response.successful()) throw new IllegalStateException("暂时无法识别承运商");
        JSONObject outer = new JSONObject(response.utf8());
    ...
    public ExpressQueryResult queryManual(
        ...
        HttpClient.Response response = new ExpressGatewayClient(context).post(
                "/api/express/timeline/source", request);
        if (!response.successful()) throw new IllegalStateException("查询失败，请稍后重试");
        JSONObject outer = new JSONObject(response.utf8());
        if (outer.optInt("code", -1) != 0) {
    ...
    static ArrayList<String> normalizedPhones(List<String> phones) {
        ...
        if (result.size() > 4) throw new IllegalArgumentException("最多可绑定 4 个手机号");
        return result;
    }
    ...
    private static JSONObject queryDetails(
        ...
        HttpClient.Response response = new ExpressGatewayClient(context).post(
                "/api/express/timeline/source", request);
        if (!response.successful()) throw new IllegalStateException("快递详情同步失败");
        JSONObject outer = new JSONObject(response.utf8());
        if (outer.optInt("code", -1) != 0) {
            throw new IllegalStateException(outer.optString("message", "快递详情同步失败"));
        }
    ...
    static JSONObject itemSummary(ExpressItem item) throws Exception {
        ...
        return new JSONObject()
                ...
                .put("provider", jd ? "JingDong" : "CaiNiao")
                .put("state", item.semantic.label)
                .put("stateNum", interface5StateNumber(item.semantic))
    ...
    static JSONObject overlay(JSONObject summary, JSONObject detail) throws Exception {
        ...
        while (keys.hasNext()) {
            ...
            if ("mailNo".equals(key) || "orderNo".equals(key)
                    || "orderId".equals(key) || "orderCode".equals(key)) continue;
    ...
    static boolean isAccountOrderRecord(JSONObject item) {
        ...
        String code = item.optString("cpCode", "");
        if (CarrierRegistry.resolve(code, item.optString("name", "")) != null) {
            String canonical = CarrierRegistry.resolve(
                    code, item.optString("name", "")).standardCode;
            if ("JD".equals(canonical) || "JDKY".equals(canonical)) return true;
        }
        String provider = (item.optString("provider", "") + " "
                + item.optString("providerName", "")).toLowerCase(Locale.ROOT);
        return provider.contains("jingdong") || provider.contains("京东");
    }
