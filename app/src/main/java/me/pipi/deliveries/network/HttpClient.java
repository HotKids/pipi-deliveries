...
final class HttpClient {
    ...
    static Response postForm(
            String url, String body, boolean redirects,
            ExpressQueryCancellation cancellation, Map<String, String> headers)
            throws Exception {
    ...
    private static Response execute(String url, String method, String contentType,
        ...
        try {
            ...
            // 调用方自己给了 User-Agent 就照用（快递100 的公开识别接口按 UA 判客户端，平台
            // Dalvik UA 会被 301 掉）；没给才回到平台 UA。
            boolean callerAgent = false;
            for (String header : headers.keySet()) {
                if ("user-agent".equalsIgnoreCase(header)) callerAgent = true;
            }
            String platformAgent = System.getProperty("http.agent", "");
            if (!callerAgent && platformAgent != null && !platformAgent.isEmpty()) {
