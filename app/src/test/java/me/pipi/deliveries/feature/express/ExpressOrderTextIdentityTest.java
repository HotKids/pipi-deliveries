...
public final class ExpressOrderTextIdentityTest {
    ...
    @Test
    public void namedCarrierIsResolvedThroughTheSameLocalTableAsIos() throws Exception {
        ...
        assertEquals("极兔速递", identity.companyName);
        assertEquals("JTSD", identity.courierCode);
    }
    ...
    @Test
    public void unknownNamedCarrierRemainsUnresolvedForRecognition() throws Exception {
        ExpressOrderTextIdentity.Identity identity =
                ExpressOrderTextIdentity.fromDetail("待出库交付某某物流，运单号为 AB12345678", "1");
        assertEquals("AB12345678", identity.waybill);
        assertEquals("某某物流", identity.companyName);
        assertEquals("", identity.courierCode);
    }
}
